    ...
    try {
        ...
        if (obj.tool_calls && Array.isArray(obj.tool_calls)) {
            for (const call of obj.tool_calls) {
                // The name might be 'write_to_file' or 'default_api:write_to_file'
                if (call.name && call.name.includes('write_to_file')) {
                    ...
                    if (args && args.TargetFile && args.CodeContent) {
                        // TargetFile might be wrapped in extra quotes
                        ...
                        if (typeof codeContent === 'string' && codeContent.startsWith('"') && codeContent.endsWith('"')) {
                            try {
                        ...
                        if (targetFile && (targetFile.includes('frontend/src') || targetFile.includes('frontend\\src') || targetFile.includes('frontend'))) {
                            writes.push({ TargetFile: targetFile, CodeContent: codeContent });
                        }
                    }
                }
            }
